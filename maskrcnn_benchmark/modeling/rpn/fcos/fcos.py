...
class FCOSHead(torch.nn.Module):
    def __init__(self, cfg, in_channels):
        ...
        cls_tower.append(
            nn.Conv2d(
                in_channels+cfg.MODEL.SEG_ON_ADD_CHANEL,
                in_channels // 2,
                kernel_size=3,
                stride=1,
                padding=1
        ...
        self.cls_logits = nn.Conv2d(
            in_channels // 2, num_classes, kernel_size=3, stride=1,
            padding=1
        )
        self.bbox_pred = nn.Conv2d(
            in_channels // 2, 4, kernel_size=3, stride=1,
            padding=1
        )
        self.centerness = nn.Conv2d(
            in_channels // 2, 1, kernel_size=3, stride=1,
            padding=1
        )
        if cfg.MODEL.FCOS.EVAL_HEAD:
    ...
    def forward(self, x, masks=None):
        ...
        for l, feature in enumerate(x):
            if masks is not None:
                ...
                #new_mask = (new_mask - 0.5) * 2
                new_feature = feature * new_mask
                #new_feature = feature
            else:
                new_feature = feature
            ...
            bbox_reg.append(torch.exp(self.scales[l](
                self.bbox_pred(self.bbox_tower(new_feature))
            )))
