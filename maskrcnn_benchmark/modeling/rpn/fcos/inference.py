...
class FCOSPostProcessor(torch.nn.Module):
    ...
    def forward_for_single_feature_map(
        ...
        max_score = box_cls.max()
        if max_score > self.pre_nms_thresh:
            candidate_inds = box_cls > self.pre_nms_thresh
        else:
            candidate_inds = box_cls >= max_score
        pre_nms_top_n = candidate_inds.view(N, -1).sum(1)
        pre_nms_top_n = pre_nms_top_n.clamp(max=self.pre_nms_top_n)

        # multiply the classification scores with centerness scores
        # I comment it for removing centerness
    ...
    def add_gt_proposals(self, proposals, targets):
        ...
        for gt_box in gt_boxes:
            ...
            gt_box.add_field("labels", torch.ones(len(gt_box), device=device, dtype=torch.int64))
...
def make_fcos_postprocessor(config, train=False):
    if not train:
        pre_nms_thresh = config.MODEL.FCOS.INFERENCE_TH
    else:
        pre_nms_thresh = config.MODEL.FCOS.TRAIN_PRE_NMS_TH
    pre_nms_top_n = config.MODEL.FCOS.PRE_NMS_TOP_N
    nms_thresh = config.MODEL.FCOS.NMS_TH
    if not train:
        fpn_post_nms_top_n = config.MODEL.FCOS.POST_NMS_TOP_N_TEST
    else:
